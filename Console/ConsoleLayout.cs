...
namespace CommandLineReimagine.Console
{
    public class ConsoleLayout
    {
        ...
        public ConsoleLayout(EntityComponentSystem ecs)
        {
            _ecs = ecs;

            Details = ecs.NewEntity("Debug Details").AddComponent<ConsoleDetails>();

            Output = ecs.NewEntity("Output").AddComponent<ConsoleOutput>();

            Input = ecs.NewEntity("Input").AddComponent<ConsoleInput>();
