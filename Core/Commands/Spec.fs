...
type StoreAccess =
    { Undo: unit -> Async<Outcome<Transaction option>>
      ...
      /// given. Only `run` is handed a `StoreAccess`, so only `run` can reach it. The
      /// line's value comes with the notes its stages gave (decision 0043), which `run`
      /// carries as its own.
      /// </remarks>
      RunLine: string -> IOutput -> CancellationToken -> Async<Outcome<Value * Note list>>
