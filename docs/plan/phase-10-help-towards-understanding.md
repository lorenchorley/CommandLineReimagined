...
Written in 10.0, numbered from 0041, all Accepted: the owner's choices of 2026-09-24.

- **0041. What the terminal says of its own is drawn apart from output.** Output is
  ...
  unaffected.
- **0042. A missing file or column names the nearest ones.** `read notes` fails as it
  ...
  predicate, where no row has `knd`, is explained with the nearest columns (see 0043).
- **0043. An empty filter explains itself.** When `where`, `find` or a view keeps no
...
- **0044. A fault may carry fixes, which the page offers as chips.** A fix is a whole
...
- **A. Suggestions and fixes (core).** G1, G3's core side. The nearest paths for a
  ...
  fixes of 0044 as notes. **Owns** `Core/Nearest.fs` (paths), the fault constructors
  in `Core/Faults.fs` that gain suggestions, and the notes the session builds from
  them. Tests in a new `SuggestionTests.fs`.
- **B. Empty answers (core).** G2. The explanation of 0043 from `where`, `find` and
...
- **C. The page.** G3's page side and G4: notes drawn as guidance panels under the
  ...
  one guidance style (0041) for notes, the guide, the banner and `copied`, and a
